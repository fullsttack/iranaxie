  ...
  <div class="flex flex-col space-y-10">
    ...
    <div class="flex justify-between items-start space-x-reverse space-x-5">
      <div class="flex justify-start items-center space-x-reverse space-x-5">
        <h3 class="text-gray-500 text-sm font-sans-medium">افزودن اکانت</h3>
      </div>
      <div class="flex justify-start items-center space-x-reverse space-x-5">
        ...
        <form @submit="create" class="flex flex-col space-y-10">
          ...
          <div class="flex flex-col space-y-5">
            <div class="form-group">
              <input v-model="form.name" type="text" class="form-control" placeholder="نام اکانت">
            </div>
            <div class="form-group">
              <input v-model="form.address" type="text" class="form-control" placeholder="آدرس اکانت">
            </div>
          </div>
          <div class="flex flex-col space-y-5">
...
export default {
  ...
  methods: {
    ...
    async create(event) {
      ...
      let response = await this.$axios.post('candy/frontend/account/create', this.form)
