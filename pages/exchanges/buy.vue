  ...
  <div class="flex flex-col space-y-10">
    ...
    <div class="flex justify-between items-start space-x-reverse space-x-5">
      <div class="flex justify-start items-center space-x-reverse space-x-5">
        <h3 class="text-gray-500 text-sm font-sans-medium">سفارش خرید</h3>
      </div>
      <div class="flex justify-start items-center space-x-reverse space-x-5">
        ...
        <div class="bg-white border border-gray-100 rounded-xl px-10 py-10">
          ...
          <div v-show="showThirdStep" class="flex flex-col space-y-10">
            ...
            <div class="flex flex-col space-y-5">
              ...
              <div class="form-group">
                <input v-model="form.wallet_address" type="text" class="form-control" placeholder="آدرس کیف پول">
              </div>
            </div>
            <div class="flex flex-col space-y-5">
              <button @click="nextStep" type="button" class="w-full bg-indigo-500 rounded-xl text-white text-sm text-center font-sans-medium px-4 py-4">مرحله بعد</button>
          ...
          <div v-show="showFourthStep" class="flex flex-col space-y-10">
            ...
            <div class="flex flex-col space-y-5">
              <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div v-if="network" class="bg-white border border-gray-100 rounded-xl px-5 py-5">
                  <div class="flex flex-col space-y-5">
                    <div class="flex justify-start items-center space-x-reverse space-x-5">
                      <h3 class="text-gray-500 text-sm font-sans-medium">کارمزد شبکه</h3>
                    </div>
                    <div class="flex justify-start items-center space-x-reverse space-x-5">
                      <p class="text-gray-500 text-sm font-sans-medium">{{ network.fee | price }}</p>
                      <p class="text-gray-500 text-sm font-sans-medium">تومان</p>
                    </div>
                  </div>
                </div>
...
export default {
  ...
  data() {
    return {
      ...
      network: null,
      form: {
        coin_id: null, card_id: null, network_id: null, wallet_address: null, amount: null, price: null
      }
    }
  },
  ...
  methods: {
    ...
    calculate() {
      ...
      if (this.coin) {
        ...
        this.form.price = (this.form.amount * this.coin.buy_price)

        if (this.network) {
          this.form.price = (this.form.price + this.network.fee)
        }
      }
    },
    ...
    async create(event) {
      ...
      let response = await this.$axios.post('candy/frontend/exchange/buy', this.form)
