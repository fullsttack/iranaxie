<template>
  <transition name="fade">
    <div class="fixed top-0 right-0 left-0 bottom-0 bg-gray-900 bg-opacity-80 px-10 py-10 z-10" v-if="dialog">
      <div class="lg:w-3/12 lg:mx-auto">
        <div class="bg-white rounded-xl shadow-lg px-5 py-5">
          <div class="flex flex-col space-y-5">
            ...
            <!-- CONTENT -->
            <div class="flex flex-col space-y-5">
              <slot></slot> <!-- WE USE THIS TO LOAD A FORM HERE -->
            </div>
            ...
            <div class="flex justify-between items-center space-x-reverse space-x-5">
              ...
              <button @click="close" type="button" class="w-full bg-red-500 rounded-xl text-white text-sm text-center font-sans-medium px-4 py-4">بستن</button>
            </div>
          </div>
        </div>
      </div>
